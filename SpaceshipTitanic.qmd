...
svm_tune_rec <- recipe(transported ~ ., data = train) %>%
  step_rm(passenger_id, name, cabin, all_nominal_predictors()) %>%  
  step_impute_median(all_numeric_predictors()) %>%  
  step_normalize(all_numeric_predictors())  

svm_tune_wf <- workflow() %>%
  add_model(svm_spec) %>%
  add_recipe(svm_tune_rec)

svm_tune_fit <- svm_tune_wf %>%
  tune_grid(
    resamples = train_folds,
    grid = 5,
...
svm_rbf_tune_rec <- recipe(transported ~ ., data = train) %>%
  step_rm(passenger_id, name, cabin, all_nominal_predictors()) %>%  
  step_impute_median(all_numeric_predictors()) %>%  
  step_normalize(all_numeric_predictors())  

svm_rbf_tune_wf <- workflow() %>%
  add_model(svm_rbf_spec) %>%
  add_recipe(svm_rbf_tune_rec)

svm_rbf_tune_fit <- svm_rbf_tune_wf %>%
  tune_grid(
    resamples = train_folds,
    grid = 5,
    metrics = metric_set(accuracy, recall, precision)
...
best_rbf_params <- svm_rbf_tune_fit %>% 
  select_best(metric = "recall")

svm_rbf_final_wf <- svm_rbf_tune_wf %>% 
  finalize_workflow(parameters = best_rbf_params)

svm_rbf_final_fit <- svm_rbf_final_wf %>% 
  fit(train)
...
The logistic regression model with hyperparameter tuning is the preferred choice because it correctly identifies approximately 87% of passengers who are truly at risk of interdimensional transport, compared to only 75% for the support vector machine (SVM) model. This higher recall rate is crucial in scenarios where missing even a small percentage of at-risk individuals could have severe consequences. Hyperparameter tuning plays a key role in optimizing the logistic regression model, as it adjusts parameters to enhance performance and ensure that the model generalizes well to new data.

## Conclusion

In conclusion, the tuned logistic regression model outperforms the SVM by correctly identifying 87% of at-risk passengers, compared to 75% with the SVM. Its improved tuning and clarity make it the preferred choice for reducing missed risks.
